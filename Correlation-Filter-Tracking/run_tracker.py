...
from CFTracker import CFTracker, CFparams
from MOSSE import MOSSEparams, MOSSETracker

# set the path to directory where you have the sequences
dataset_path = '../vot2014'#'workspace-dir/sequences' # set to the dataset path on your disk
sequence = input('Name of sequence to track: ')  # choose the sequence you want to test
...
tracker_name = input("Tracker to use: (mosse, cf) ")
if tracker_name == "mosse":
    parameters = MOSSEparams()
    tracker = MOSSETracker(parameters = parameters)
else:
    parameters = CFparams()
    tracker = CFTracker(parameters = parameters)
...
while frame_idx < sequence.length():
    ...
    if frame_idx == init_frame:
        ...
        track_time.append(time.time() - t_)
        time_all += time.time() - t_
    ...
    # cv2.imwrite("sequence/frame%03d.png" % frame_idx, img)
...
print('Average init time:', np.mean(init_time))
print('Average track time:', np.mean(track_time))